...
sub_dat <- subset(data, data$Sample == "COG-N-625x-D", c(colnames(data)))
tx_dat <- subset(sub_dat, sub_dat$transcript == "ENST00000480283.1", c(colnames(sub_dat)))
...
a <- ggplot(sub_dat, aes(exon_n, FPKM)) +
  ...
  labs(title = "FPKM for exons in model COG-N-625x") +
  facet_wrap(. ~ sub_dat$transcript, nrow = 3) +
  theme_bw()

ggsave(paste0(Sys.Date(),"-exon-number-all-TXs-COG-N-625x-.pdf"), a, width=45, height=20, units = "cm")
...
n_pages_needed = 13
pdf("2019-01-03-NBL-Tx-ATRX.pdf", width=15, height=9)
